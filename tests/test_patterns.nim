...
suite "patterns":
  ...
  test "checkers should repeat in x":
    let pat = newCheckersPattern(white, black)
    check(pat.colorAt(point(0, 0, 0)) =~ white)
    check(pat.colorAt(point(0.99, 0, 0)) =~ white)
    check(pat.colorAt(point(1.01, 0, 0)) =~ black)

  test "checkers should repeat in y":
    let pat = newCheckersPattern(white, black)
    check(pat.colorAt(point(0, 0, 0)) =~ white)
    check(pat.colorAt(point(0, 0.99, 0)) =~ white)
    check(pat.colorAt(point(0, 1.01, 0)) =~ black)

  test "checkers should repeat in z":
    let pat = newCheckersPattern(white, black)
    check(pat.colorAt(point(0, 0, 0)) =~ white)
    check(pat.colorAt(point(0, 0, 0.99)) =~ white)
    check(pat.colorAt(point(0, 0, 1.01)) =~ black)
